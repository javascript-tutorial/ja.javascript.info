...
関数はアクションです。そのため、それらの名前は通常は動詞です。それは簡潔にすべきすが、関数がすることをできるだけ正確に表現してください。そして、コードを読む人が正しい手がかりを得られるようにします。
...
- `checkPermission` -- `アクセス許可/拒否` のチェックを行い、メッセージを表示します(チェックのみを実行し、その結果を返すべきです)
