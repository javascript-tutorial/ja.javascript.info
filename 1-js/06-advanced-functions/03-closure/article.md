...
## レキシカル/語彙環境(Lexical Environment) [#Lexical Environment]
...
## コードブロックとループ、IIFE(即時実行関数) [#Code blocks and loops, IIFE]
