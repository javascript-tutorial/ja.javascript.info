...
2. そうでなければ、`pow(x, n)` は `x * pow(x, n - 1)` と表現することができます。数学的には、<code>x<sup>n</sup> = x * x<sup>n-1</sup></code> と書けます。これは *再帰的なステップ* と呼ばれます: タスクをより単純なアクション(`x` の乗算)と、同じタスクの呼び出し(より小さい `n` での `pow`)に変換します。次のステップではそれをさらに単純化し、`n` が `1` になるまで単純化されます。
