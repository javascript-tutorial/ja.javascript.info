...
私たちが数値について現在知っていることをおさらいしならがら発展させて行きましょう。

## 数値を書く多くの方法 

1億を書く場合を想像してください。明白な方法は次の通りです:

```js
let billion = 1000000000;
```

しかし実際には、通常は間違えやすいので何度もゼロを書くのを避けます。また我々は怠け者です。1億を `"1bn"` としたり、7億3千万を `"7.3bn"` と書いたりします。大きな数値を表現する場合にはだいたいこれが当てはまります。
...
2. 一時的に数値を整数に変換して戻す方法があります。 それはこのように動作します:
