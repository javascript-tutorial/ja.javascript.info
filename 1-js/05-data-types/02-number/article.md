...
## 数値を書く多くの方法 [#more-ways-to-write-a-number]

1億を書かないといけないと想像してください。明白な方法は次の通りです:

```js
let billion = 1000000000;
```

しかし実際には、間違えやすいので通常は何度もゼロを書くのを避けます。また我々は怠け者です。1億を `"1bn"` としたり、7億3千万を `"7.3bn"` のように書いたりします。大きな数値を表現する場合にはだいたいこれが当てはまります。

JavaScriptでは、数値に文字 `"e"` を追加し、ゼロの数を指定することで数値を短くします。:

```js run
let billion = 1e9;  // 1 億, 文字通り: 1 と 9 個のゼロ

alert( 7.3e9 );  // 7.3 億 (7,300,000,000)
```
...
さて、非常に小さい数値を書いてみましょう。1マイクロ秒(100万分の1秒):

```js
let ms = 0.000001;
```

ちょうど前に書いたように、`"e"` が役立ちます。明示的にゼロを書くのを避けたい場合、このように書くことができます:
...
つまり、`"e"` の後の負値は `1` と与えられたゼロの数と数値を割ったものになります。
...
[16進数](https://en.wikipedia.org/wiki/Hexadecimal) の数値は色、エンコード文字やその他多くのものを表現する文字として、JavaScriptで広く使われています。もちろん、それらを短く書く方法があります: `0x` とそれに続いて数値を書きます。
...
またこのようにも書けます `(123456).toString(36)`.
```
...
少しはっきりしないかもしれませんが、よく発生するのは精度の低下です。
...
なんと!ここでは誤った比較よりも多くの驚きがあります。e-ショッピングのサイトを作っており、訪問者が `$0.10` と `$0.20` の商品をカートに入れたと想像してください。注文の総額は `$0.30000000000000004` になります。誰もがそれに驚くでしょう。
...
数値形式 IEEE-754 は、可能な限り近い数値に丸めてこれを解決します。 これらの丸めルールでは、通常 "小さな精度損失" を見ることができないので、数値は `0.3` と表示されます。 しかし、損失は依然として存在することに注意してください。
...
Javascriptはこのようなイベントではエラーをトリガーしません。数値を目的のフォーマットに合わせるよう最善を尽くしますが、残念ながらこのフォーマットは十分大きいものではありません。
````
...
ほとんどの場合、区別は目立ちません。なぜなら演算子はそれらを同じものとして扱うためです。
```
...
しかし、実際には、CSS で `"100px"` や `"12pt"` のような単位を持つ値を持つことが頻繁にあります。また、多くの国では、通貨記号が金額の後にあるので、`"19€"` と言った値があり、その中から数値を抽出したいと考えています。
